...
function ScrollProgress() {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 h-[2px]">
      <div style={{ width: `${pct}%`, backgroundColor: VIOLET, height: '100%', transition: 'width 0.08s linear' }} />
    </div>
  )
}
    ...
    <main style={{ backgroundColor: BG }} className="text-white antialiased">
      ...
      <section className="px-8 md:px-16 lg:px-24 py-12 bg-white">
        <motion.div initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }}>
          ...
          <p className="text-purple-600 text-xs tracking-[0.2em] uppercase font-semibold mb-3">Overview</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-zinc-100 rounded-2xl overflow-hidden">
            {cs.metrics.map(m => (
              <div key={m.label} className="bg-white px-6 py-6">
                <div className="text-3xl font-bold mb-1" style={{ color: PURPLE }}>{m.value}</div>
      ...
      <section className="px-8 md:px-16 lg:px-24 py-12 bg-[#f9f9f7]">
        <motion.div initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }}>
          ...
          <p className="text-purple-600 text-xs tracking-[0.2em] uppercase font-semibold mb-3">Process</p>
          {cs.processIntro && (
          ...
          <div className="space-y-8">
            {cs.process.map((step, i) => (
              <motion.div
                ...
                style={{ borderColor: `${PURPLE}50` }}
              >
                <p className="text-purple-600 text-xs tracking-widest uppercase mb-2 font-semibold">{step.num}</p>
      ...
      <section className="px-8 md:px-16 lg:px-24 py-12 bg-[#f9f9f7]">
        <motion.div initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }}>
          ...
          <p className="text-purple-600 text-xs tracking-[0.2em] uppercase font-semibold mb-3">{cs.conclusion.heading}</p>
          <div className="max-w-3xl space-y-5">
            {cs.conclusion.paragraphs.map((p, i) => (
              <p key={i} className={`leading-relaxed ${
                p.startsWith('"') ? 'border-l-2 border-purple-300 pl-4 text-zinc-500 italic text-sm' : i === 0 ? 'text-zinc-800 text-base' : 'text-zinc-500 text-sm'
              }`}>{p}</p>
            ))}
          </div>
      ...
      <section className="px-8 md:px-16 lg:px-24 py-16" style={{ backgroundColor: BG }}>
        ...
        >
          <div>
            <p className="text-purple-400 text-xs uppercase tracking-[0.2em] font-medium mb-4">{cs.client} · Full Case Study</p>
