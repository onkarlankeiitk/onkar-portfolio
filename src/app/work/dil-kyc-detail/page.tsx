  ...
  return (
    <section className="overflow-hidden" style={{ backgroundColor: NAVY }}>
      ...
      <div className="flex flex-col-reverse md:grid md:grid-cols-[1fr_1.2fr] gap-10 md:gap-16 px-8 md:px-16 lg:px-24 pt-10 md:pt-14 pb-14 md:pb-20 items-start">
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-4 pt-2">
            {[
              ...
              <div key={m.label}>
                <p className="text-2xl font-bold mb-0.5 text-green-400">{m.value}</p>
            ...
            <div className="flex flex-wrap gap-1.5">
              {TEAM.map(m => (
                <a key={m.name} href={m.url} target="_blank" rel="noreferrer"
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-zinc-700 bg-zinc-800/50 hover:border-green-500/50 transition-colors group">
                  <span className="text-white text-xs font-medium">{m.name}</span>
                  <span className="text-zinc-600 text-[10px]">·</span>
                  <span className="text-zinc-500 text-xs">{m.role}</span>
                  <svg className="w-2.5 h-2.5 text-zinc-600 group-hover:text-green-400 transition-colors" viewBox="0 0 24 24" fill="currentColor"><path d="M19 3A2 2 0 0 1 21 5V19A2 2 0 0 1 19 21H5A2 2 0 0 1 3 19V5A2 2 0 0 1 5 3H19M18.5 18.5V13.2A3.26 3.26 0 0 0 15.24 9.94C14.39 9.94 13.4 10.46 12.92 11.24V10.13H10.13V18.5H12.92V13.57C12.92 12.8 13.54 12.17 14.31 12.17A1.4 1.4 0 0 1 15.71 13.57V18.5H18.5M6.88 8.56A1.68 1.68 0 0 0 8.56 6.88C8.56 5.95 7.81 5.19 6.88 5.19A1.69 1.69 0 0 0 5.19 6.88C5.19 7.81 5.95 8.56 6.88 8.56M8.27 18.5V10.13H5.5V18.5H8.27Z" /></svg>
                </a>
              ))}
            </div>
          </div>
        ...
        >
          <img
            src="/case-studies/dil-kyc/detail-banner.png"
...
function Team() {
  ...
  return (
    <section id="team" style={{ backgroundColor: NAVY }} className="px-8 md:px-16 lg:px-24 py-12">
      <motion.div
        ...
        <div className="flex flex-wrap gap-2">
          {TEAM.map((member) => (
            <a key={member.name} href={member.url} target="_blank" rel="noreferrer"
              className="inline-flex items-center gap-2 px-3.5 py-2 rounded-full border border-zinc-700 bg-zinc-800/50 hover:border-green-500/50 hover:bg-zinc-800 transition-colors group">
              <span className="text-white text-xs font-medium">{member.name}</span>
              <span className="text-zinc-600 text-[10px]">·</span>
              <span className="text-zinc-500 text-xs">{member.role}</span>
              <span className="text-zinc-600 group-hover:text-green-400 transition-colors">{liIcon}</span>
            </a>
          ))}
        </div>
      </motion.div>
...
export default function DILKYCDetail() {
  return (
    <main className="bg-white text-zinc-900 antialiased">
      <PasswordGate accentColor="#0fa475" />
      {/* <Nav /> */}
      <StickyNav />
      <Hero />
